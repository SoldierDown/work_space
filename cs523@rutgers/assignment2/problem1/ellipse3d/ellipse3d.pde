 ...
 for(int i=1;i<=10;i++){
  ...
  if(abs(phi_interpolation(x,y,z))<epsilon){
    ...
    while(abs(phi_interpolation(x,y,z))>epsilon){
      ...
      dy=dy/dl;
      dz=dz/dl;
      x-=0.5*abs(phi_interpolation(x,y,z))*dx;
      y-=0.5*abs(phi_interpolation(x,y,z))*dy;
      z-=0.5*abs(phi_interpolation(x,y,z))*dz;
...
{
  float ph= sqrt(pow((x_-width/2)/a,2)+pow((y_-height/2)/b,2)+pow((z_-height/2)/c,2))-1;
